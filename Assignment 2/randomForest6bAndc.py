...
# Code takes about 20 secs on an i7
...
def run_decision_tree():
    ...
    numberOfTrees = 21
    #llop for multiple values
    for a in range(1,56,2):
        forest = []
        forestData = []
        #new data set
        ...
        print("m = ", a,",test error = ", 1-    iterAccuraccy)
        #    oob error
        oobResults = []    
        for x in trainData:
            numZeros = 0
            numOnes = 0
            for z in range(numberOfTrees):
                if(not(x in forestData[z])):
                    result = forest[z].classify(x[:-1])
                    if(int(result) == 0):
                        numZeros += 1
                    else:
                        numOnes += 1
                if(numOnes > numZeros):
                    oobResults.append(1 == int(x[-1]))
                else:
                    oobResults.append(0 == int(x[-1]))  
        iterAccuraccy= float(oobResults.count(True))/float(len(oobResults))
        print("m = ",a,", oob error = ",1- iterAccuraccy)
